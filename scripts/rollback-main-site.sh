#!/usr/bin/env bash
# =============================================================================
# rollback-main-site.sh
#
# Restores the main site from a timestamped backup created by deploy-main-site.sh.
#
# Backups are created automatically before each deployment at:
#   <deploy-path>.backup.<timestamp>
#
# Usage:
#   bash scripts/rollback-main-site.sh                      # restore most recent backup
#   bash scripts/rollback-main-site.sh 1710600000           # restore specific backup
#   DRY_RUN=true bash scripts/rollback-main-site.sh         # preview only
#
...
REMOTE_PATH="$DEPLOY_MAIN_SITE_PATH"
DRY_RUN="${DRY_RUN:-false}"
SSH_ARGS=(-p "$DEPLOY_PORT" -i ~/.ssh/deploy_key)

echo "Looking for backups of main-site at $DEPLOY_HOST:$REMOTE_PATH..."
...
echo "Rollback of main-site complete."
